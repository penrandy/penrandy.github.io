...
var visualize_activations = function(net, elt) {
  ...
  for (var i = 0; i < N; i++) {
    ...
    title_wrap.appendChild(title_div);

    if (L.layer_type === 'conv') {
      var t = 'filter size ' + L.filters[0].sx + 'x' + L.filters[0].sy + 'x' + L.filters[0].depth + ', stride ' + L.stride;
      title_wrap.appendChild(document.createTextNode(t));
      title_wrap.appendChild(document.createElement('br'));
    }
    ...
    layer_div.appendChild(title_wrap)
...
var toggle_pause = function() {
  ...
  if (paused) {
    //btn.value = 'resume'
    imgPlay.src ="../images/svg/pause.svg";
  } else {
    //btn.value = 'pause';
    imgPlay.src ="../images/svg/play.svg";
  }
}
